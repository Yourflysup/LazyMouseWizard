...
#include "module/impl/humanInput.hpp"
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int) {
    ...
    while (msg.message != WM_QUIT) {
        ...
        if (!waitingKey && (GetAsyncKeyState(goblin.toggleKey) & 1))
            goblin.setStatus(!goblin.getStatus());
        ...
        ImGui::BeginChild("key_row", {panelW, rowH}, false, ImGuiWindowFlags_NoScrollbar);

        const float y = (rowH - ImGui::GetFrameHeight()) * 0.5f;
        ImGui::SetCursorPosY(y);

        char btn[64];
        waitingKey
            ? strcpy_s(btn, "Press a key...")
            : sprintf_s(btn, "Current Key: %s", KeyName(goblin.toggleKey));
        ...
        const ImVec4 state = goblin.getStatus()
            ? ImVec4(0.25f, 0.85f, 0.40f, 1.f)
            : ImVec4(0.90f, 0.25f, 0.25f, 1.f);

        ImGui::SetCursorPosY(y + 2.f);
        ImGui::TextColored(state, goblin.getStatus() ? "ENABLED" : "DISABLED");
        ...
        ImGui::SliderInt("##min", &goblin.minCPS, 1, 25, "Min %d");
        ImGui::SliderInt("##max", &goblin.maxCPS, 1, 25, "Max %d");
        ImGui::PopItemWidth();

        if (goblin.minCPS > goblin.maxCPS)
            goblin.minCPS = goblin.maxCPS;
